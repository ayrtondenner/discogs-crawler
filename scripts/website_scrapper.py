...
def _navigate_to_element(web_element: WebElement):
    web_element._parent.execute_script(
        "arguments[0].scrollIntoView({block: 'center'});", web_element
    )
    ...
    try:
        ...
        modal = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located(
                (By.XPATH, '//div[@class="modal" and @role="dialog"]')
            )
        )
        close_container = modal.find_element(
            By.XPATH, './/div[contains(@class, "close-container")]'
        )
        close_button = close_container.find_element(
            By.XPATH, './/button[contains(@class, "close-button")]'
        )
        WebDriverWait(driver, 10).until(
            lambda d: close_button.is_enabled() and close_button.is_displayed()
        )
        close_button.click()
    ...
    try:
        ...
        text_only_button = WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable(
                (By.XPATH, '//button[@title="Switch to text-only view"]')
            )
        )
        ...
        select_container = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located(
                (By.XPATH, '//div[contains(@class, "selectContainer")]')
            )
        )
        ...
        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located(
                (By.XPATH, '//table[contains(@class, "releases")]')
            )
        )
        ...
        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located(
                (By.XPATH, '//table[contains(@class, "releases")]/thead')
            )
        )
    except Exception as e:
...
def _find_albums_table(driver: ChromiumDriver) -> WebElement:
    try:
        ...
        album_thead = next(
            child for child in table_children if child.text.strip() == "Albums"
        )
        assert album_thead.tag_name == "thead", (
            "Expected 'Albums' header to be a thead element"
        )
...
def _find_expand_button(tr: WebElement) -> Optional[WebElement]:
    try:
        ...
        if span is None:
            return None  # Skip rows without a span
        button = span.find_element(
            By.XPATH, './/button[contains(@class, "versionsButton")]'
        )
        return button
    except Exception as e:
...
def _expand_all_versions(driver: ChromiumDriver):
    ...
    try:
        trs = WebDriverWait(driver, 10).until(
            lambda d: albums_table.find_elements(By.TAG_NAME, "tr")
        )
        expand_count = 0
        ...
        for tr in tqdm(trs, desc="Expanding versions", unit="row"):
            ...
            assert expand_button.get_attribute("aria-expanded") == "false", (
                "Expected aria-expanded to be 'false' before click"
            )
            ...
            expand_button.click()
            WebDriverWait(driver, 10).until(
                lambda _: expand_button.get_attribute("aria-expanded") == "true" # pyright: ignore[reportOptionalMemberAccess]
            )
...
def extract_album_row(tr: WebElement, is_version: bool = False) -> AlbumRow:
    ...
    if expand_button is None:
        WebDriverWait(tr, 10).until(
            EC.presence_of_element_located(
                (By.XPATH, './/div[contains(@class, "formatsContainer")]')
            )
        )
        formats_div = tr.find_element(
            By.XPATH, './/div[contains(@class, "formatsContainer")]'
        )
        assert "LP" in str(formats_div.get_attribute("innerHTML")), (
            "Expected 'LP' in formatsContainer div"
        )
    ...
    if expand_button is None:
        ...
        assert cat_div.get_attribute("tabindex") == "0", (
            "Expected tabindex to be '0' in cat_div"
        )
    ...
    try:
        ...
        for tr in tqdm(trs, desc="Extracting all rows", unit="row"):
            ...
            if "textOnlyRow" in tr_class:
                ...
                album_row = next(
                    row for row in album_rows if row.title == expanded_album_row.title
                )
                ...
                for i in range(close_versions_tr_index - 1, -1, -1):
                    ...
                    # Original text-only row
                        ...
                        raise ValueError(
                            f"Unexpected check tr class: {current_tr_class}"
                        )

                assert text_only_tr is not None, (
                    "Could not find preceding row with textOnlyRow class"
                )
...
def extract_from_webpage():
    ...
    try:
        ...
        assert len(album_rows) == original_trs_len, (
            f"Expected {original_trs_len} album rows, but found {len(album_rows)}"
        )
        assert original_trs_len + (expand_count * 2) == expanded_trs_len, (
            f"Expected {original_trs_len} original rows + {expand_count * 2} expanded rows, but found {expanded_trs_len}"
        )

        # Manually add an album that doesn't exist in the table
        new_album_row = AlbumRow(
            title="Planeta Lamma", ids={"DEPL108, musique brut 2"}, years={"2022"}
        )
